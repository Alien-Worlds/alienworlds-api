...
export class AssetAggregator {
    ...
    async process (asset_id: string) {
        ...
        const asset = await asset_collection.findOne(query, {sort: { sequence: -1 }})
        // console.log(asset)
        ...
        if (asset.template_id > 0){
            ...
            const asset_first = await asset_collection.findOne(query, {sort: { sequence: 1 }})
            const ids = await asset_collection.distinct('asset_id', { template_id: template.template_id, sequence: { $lte: asset_first.sequence } } )
            asset.mint_number = ids.length
        }
        else {
            asset.data = {...asset.mutable_serialized_data, ...asset.immutable_serialized_data}
        }
